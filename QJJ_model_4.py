...
def test_model_4():
    ...
    # Cf[i,p] = Total shipout Cost to ship part p from plant i 
    data = [[4, 1, 2, 3, 1, 2, 1, 3, 3],    #'Hamburg'
            [2, 3, 4, 2, 4, 4, 3, 3, 3],    #'Helsinki'
            [1, 4, 1, 4, 2, 4, 1, 4, 2]]    #'London'
    ...
    # Carbons[i,j,p] = Cost to ship part p from plant i to destination j
    data = [[[4, 3, 3, 4, 4, 1, 5, 3, 1],   #'Hamburg','SG'
            [4, 4, 5, 4, 4, 3, 3, 2, 1],   #'Hamburg','Osk'
            [1, 5, 1, 3, 5, 5, 3, 5, 1]],  #'Hamburg','Sh'
            [[1, 1, 3, 1, 4, 4, 2, 5, 2],   #'Helsinki','SG'
            ...
            [5, 1, 1, 3, 1, 3, 1, 1, 5],   #'London','Osk'
            [1, 3, 5, 1, 1, 4, 1, 5, 3]]]  #'London','Sh'
    Carbons = {(i,j,p):data[a][b][c] for a,i in enumerate(Plants) for b,j in enumerate(Dests) for c,p in enumerate(Parts)}
    ...
    targetCarbonRate = 0.6
    Vspeed = 1
    optz_model_4(Parts, Inspections, Plants, Vessels, Dests, A, B, Cship, Rm, Cplant, Distances, 
                 VesselSchedule, Rp, Rn, H, Cinsp, CarbonEmissions, Vspeed)
        
Preoutputs = None
def optz_model_4(Parts, Inspections, Plants, Vessels, Destinations, 
                 AvailabilityPartPlant, AvailabilityInspPort, TransCost, PartsInInsp, TransOutCost, 
                 Distances, VesselSchedule, VesselPartDemand, VesselInspDemand, HoldingCsot, 
                 InspectionCost, CarbonEmissions, VehicleSpeed, budget=0, details=True):
    ...
    S = VesselSchedule
    LM = 100000
    # CarbonEs[i,j,p] = kgs carbon emissions per km for shipping part p from plant i to port j
    CarbonEs = CarbonEmissions
    CarbonEs = {(i,j,p): CarbonEmissions[i,j,p] * Distances[i,j] for i in Plants for j in Dests for p in Parts}    
    ...
    if budget == 0:
        budgetLimit = 0
        maxHoldingTime = 10000
        for p in Parts:
            cc = max([v for (i,j,pp),v in Cship.items() if A[i,p] == 1 and p == pp])
            cm = max([v for (j,pp),v in H.items() if pp == p]) * maxHoldingTime
            budgetLimit += (cc + cm) * sum([1 for k in Vessels if p in Rp[k]])
        budgetLimit += sum([v for k,v in Cplant.items()])
        for n in Inspections:
            cc = max([v for (j,nn),v in InspectionCost.items() if B[j,n] == 1 and n == nn])
            budgetLimit += cc * sum([1 for k in Vessels if n in Rn[k]])
    else:
        budgetLimit = budget
    ...
    m.setObjective(
        gp.quicksum(CarbonEs[i,j,p] * x[i,j,k,p] for i,j,k,p in PlantPortVesDParts),
                            GRB.MINIMIZE)
    ...
    # Carbon Constraints
    # total shipping cost + holding cost
    val7 = m.addVar(vtype=GRB.CONTINUOUS, name='val7')
    c141 = m.addConstr((val7 == gp.quicksum(Cship[i,j,p]*x[i,j,k,p] + H[j,p]*w[j,k,p]*x[i,j,k,p]
                    for i,j,k,p in PlantPortVesDParts)), name='c141')
    # total shipout cost
    val8 = m.addVar(vtype=GRB.CONTINUOUS, name='val8')
    c142 = m.addConstr((val8 == gp.quicksum(Cplant[i,j]*z[i,j] for i,j in PlantPorts)), name='c142')
    # total inspection cost
    val9 = m.addVar(vtype=GRB.CONTINUOUS, name='val9')
    c143 = m.addConstr((val9 == gp.quicksum(Cinsp[j,n]*y2[j,k,n] for j,k,n in PortVesDInsps)), name='c143')
    
    # c144 = m.addConstr((val7 + val8 + val9 <= budgetLimit), name='c144')
    c144 = m.addConstr((val7 + val9 <= budgetLimit), name='c144')

    # Save model for inspection/debugging
    m.write('models/model_4.lp')
    
    # m.Params.timeLimit = 120
    m.Params.MIPFocus = 3
    m.Params.ImproveStartGap = 1
    ...
    if m.status == GRB.Status.OPTIMAL:
        ...
        # total_cost = val7.x + val8.x + val9.x
        total_cost = Cost_shipping + Cost_holding + Cost_plant + Cost_Inspection
        ...
        print(f"Total carbon emission  = {Carbon_emmision}")
        ...
        obj = {'Cost': ['Carbon emission', 'Total cost', 'Total shipping cost', 'Total holding cost', 'Total ship out cost', 'Total inspection cost'], 
            'Value': [Carbon_emmision, total_cost, Cost_shipping, Cost_holding, Cost_plant, Cost_Inspection]}
        for k in Vessels:
            for j in Ports_Ves[k]:
                if y[j,k].x == 1:
                    for i,p in PlantDParts_PortVes[j,k]:
        ...
        global Preoutputs, counter
        Preoutputs = [obj, sol_trans, sol_insp]
        if counter > 0:
            sheets = [f'{sheet}_{counter}' for sheet in sheets]
        write2Excel('results/model 4.xlsx', [obj, sol_trans, sol_insp], sheets)
    return int(m.status == GRB.Status.OPTIMAL)


def run_model_4_rounds(Parts, Inspections, Plants, Vessels, Destinations, 
                 AvailabilityPartPlant, AvailabilityInspPort, TransCost, PartsInInsp, TransOutCost, 
                 Distances, VesselSchedule, VesselPartDemand, VesselInspDemand, HoldingCsot, 
                 InspectionCost, CarbonEmissions, VehicleSpeed, minbudget=0, maxbudget=0, rounds=1, details=False):
    global Preoutputs, counter
    if maxbudget == 0:
        res = optz_model_4(Parts, Inspections, Plants, Vessels, Destinations, 
                 AvailabilityPartPlant, AvailabilityInspPort, TransCost, PartsInInsp, TransOutCost, 
                 Distances, VesselSchedule, VesselPartDemand, VesselInspDemand, HoldingCsot, 
                 InspectionCost, CarbonEmissions, VehicleSpeed, 0, False)
        if res == 0:
            return res
        maxbudget = Preoutputs[0]['Value'][1]
    if minbudget >= maxbudget:
        budgets = [minbudget]
        rounds = 1
    else:
        if maxbudget - minbudget < rounds - 1:
            rounds = maxbudget - minbudget + 1
        step = (maxbudget - minbudget) / (rounds - 1)
        budgets = []
        for i in range(rounds - 1):
            budgets.append(minbudget + i * step)
        budgets.append(maxbudget)
    
    if rounds > 2:
        for i in range(rounds):
            if i == 0 or i == rounds - 1:
                continue
            if int(budgets[i]) > budgets[i-1]:
    ...
    for b in budgets:  
        res += optz_model_4(Parts, Inspections, Plants, Vessels, Destinations, 
                 AvailabilityPartPlant, AvailabilityInspPort, TransCost, PartsInInsp, TransOutCost, 
                 Distances, VesselSchedule, VesselPartDemand, VesselInspDemand, HoldingCsot, 
                 InspectionCost, CarbonEmissions, VehicleSpeed, b, details)
        counter += 1
    counter = 0
    return int(res > 0)
